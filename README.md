...
$ jobs
```
![jobs 명령어 스크린샷](./images/jobs.png)
```bash
$ fg %1
```
![fg 명령어 스크린샷](./images/fg.png)

## 4. `kill` 명령어: 프로세스에 시그널 보내기

`kill`명령어는 지정된 **PID** (프로세스 ID)를 가진 프로세스에게 **시그널** 을 보냅니다. 가장 흔하게는 프로세스를 종료(kill)하는 데 사용되지만, 다른 제어 시그널도 보낼 수 있습니다.

### 주요 시그널
| 시그널 번호 | 시그널 이름 | 설명 |
| :--- | :--- | :--- |
| **1** | **SIGHUP** | 터미널 연결 끊김 또는 설정 파일 재로드 요청 |
| **9** | **SIGKILL** | **강제종료** . 프로세스가 무시할 수 없는 가장 강력한 종료 시그널 |
| **15** | **SIGTERM** | **정상종료** 요청. 프로세스가 종료 전 정리 작업을 할 수 있는 기회를 줌(기본값) |
| **19** | **SIGTOP** | 프로세스를 일시 중지 (Stopped) |

### 사용 방법
```bash
$ kill [옵션:시그널] [PID 또는 %Job_ID]
```
- **참고**: 시그널 이름 앞에 `-`를 붙이거나, 시그널 번호만 사용할 수도 있습니다.
